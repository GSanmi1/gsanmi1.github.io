...
But these sets are characterized as algebraic structures with their own properties. For example:
...
- Then, again, we can expand $\mathbb{Z}$ to $\mathbb{Q}$ along with the notion of a *fraction* in order to ensure that every element has a multiplicative inverse; this is called a *field*. But then, there exist equations, like $x^2 - 2 = 0$, with no solutions in $\mathbb{Q}$.

- Then, we expand $\mathbb{Q}$ to $\mathbb{R}$ in order to give a solution for that equation, forming the notion of irrational numbers; $\sqrt 2, \pi, etc$. This is called a *complete field*. But again, $x^2 + 1 = 0$, for a different reason, also has no roots.

- Now, we expand from $\mathbb{R}$ to $\mathbb{C}$. This now gives a completely valued (concept of analysis), algebraically closed (concept of algebra) field.
...
- $\circ : G \times G \to G$ is a binary operation satisfying the following statements:
    ...
    4. **Inverse element**: $\forall a \in G \  \exists a^{-1} \in G : a \circ a^{-1} = a^{-1} \circ a = e$. This is the *undo* possibility any group allows.
...
- **Cancelation**:
    ...
    We are going to demonstrate it partially, composing on the left:
...
2. The addition and multiplicative operations satisfy the *distributive* property:
...
- *Commutativity*: $\begin{cases} \ x + y = y + x \\\\ \ x·y = y·x \end{cases}$
...
1. First, let's prove that $x·0 = 0$. Taking the compatibility property of $F$, then:
...
3. Let's also observe that: $1_K = 1_F \wedge 0_K = 0_F \ \ \forall K \subset F : \Set{K, +, \ ·}$ is a field.
    ...
    In $F$, the identity is $1\_F$, then due to the uniqueness of this identity in $F$, $1\_K = 1\_F$ for the elements of $K$ in $F$, so isolating $K$ from $F$ necessarily $1\_{F} \in K$, and the same can be applied to $0$, meaning that any subfield inherits the identity of the superfield.
...
Then, any tuple $(x_1,\cdots,x_n) \in F^n$ that satisfies all the constraints simultaneously is called a *solution* of the system.
...
Thus, the system $M$ describes those $x \in F^n$ that satisfy the intersections of the $S_{P_i}$ sets:

$$S:= \bigcap_{i=1}^m S_{P_i} = \Set{x \in F^n \ \vert \ M := \bigwedge_{i=1}^m P_i(x) \equiv \top}$$

We say that two equation systems, $M, M'$ are *equivalent* $M \equiv M'$ if they describe the same solution set $S$:
...
1. **Multiply by a scalar and replace**: $P \in M \wedge \lambda \in F\setminus \Set{0} \implies S_{\lambda P} = S_P $
...
2. **Add two equations**: $P,Q \in M \wedge \gamma ,\lambda \in F \setminus \Set{0} \implies S_{\gamma P + \lambda Q} \cap S_Q = S_P \cap S_Q $
    ...
    Implying that the system: $ M' := (\gamma P + \lambda Q) \wedge \displaystyle\bigwedge_{i=1}^m P_i \equiv\bigwedge_{i=1}^m P_i = M$ 
...
- **Commutativity**:
    ...
    And both expressions are equivalent due to commutativity in $\mathbb{Q}$
...
First, to demonstrate that $M$ and $M'$ are equivalent we can solve it by describing the sets $S$ and $S'$ and demonstrating that $S = S'$.
...
Solving for $x_1$ it is obvious that both systems are not equivalent since the solution sets $S_M$ and $S_{M'}$ are not the same.
...
- Being $\Set{F,+}$, then:
    ...
    - **Inverse**: For every element $a \in F$, $a$ is its own inverse:

        $$ 0 + 0 = 0 \wedge 1 + 1 = 0$$

        <br>

- Being $\Set{F \setminus \Set{0}, \ ·}$, then let's demonstrate that it is a group.
    ...
    - **Identity & Inverse**: Check that $1$ is at the same time the identity and its own inverse: $a·1 = 1·a = a \ \ \forall a \in F \setminus \Set{0}$
...
Geometrically it is easy to see that there is a linear combination dependency between the direction vectors:

$$\alpha(u_2,-u_1) + \beta(v_2,-v_1) = (w_2,-w_1)$$

$$\lambda(u_2,-u_1) + \gamma(v_2,-v_1) = (p_2,-p_1)$$

Then, these scalars exist and can be calculated by forming an equation system with the coordinates:
...
- $A$ is the **matrix of coeficients**:
    ...
    Strictly speaking, the rectangular array displayed above is not a matrix, but a representation of a matrix, a concrete display or codification of the mathematical object we haven't presented yet.
...
All these functions $\mathcal{E}$ eventually relate $A$ with the matrices $E_{\lambda r}^r, E_{r + \lambda s}^r, E_{r  \leftrightarrow s}$
...
This will make sense when we present the matrix product and check that the resulting system of equations is the result of applying a property that respects the equivalence as we saw at the start of this post.
...
Let's observe that the term "equivalence" in row-equivalence is specifically chosen because, if we consider again $A, A' \in M_{m \times n-1} (F)$ and $M, M'$ two equation systems such that: $M := AX = Y \wedge M' := A'X = Y'$, then:
...
- $\Leftarrow$ 
    ...
    we can craft the systems $[AX = Y]:=\bigwedge P\_i$ and  $[A'X=Y'] :=\bigwedge P'\_i$ and $P\_i = P'\_i \ \ i \neq r \wedge P\_r = \lambda P\_r$ and, as we saw above, $S\_P = S\_{\lambda P} \implies [AX = Y] \equiv [A'X = Y']$
...
These are equivalent, being $M'$ trivial. 
...
1. **Find all solutions to the system of equations:**
    ...
    Meaning basically that this is a redundant system of two equations whose simplified form is:
...
4. **Find a row-reduced matrix row-equivalent to:**
    ...
    **Defined over the field $\mathbb{C}$**
    ...
    But this is not correct since the expression $-\frac{1}{2i + \frac{1+i}{i}} - \frac{1}{-2 + \frac{1+i}{í}} = 0$ ($\frac{1+i}{i}=1-i$) so we can't use it to perform the row operation that makes $a_{23} = 0$ and thus we cannot use $r_2$ to make $a_{12}=0$. This is a friendly reminder that $\mathbb{C}$ is not a trivial extension of $\mathbb{R}$ and your visual intuition can betray you when deciding whether some complex expression does not collapse to 0. 
...
6. **Let $A \in M_2(\mathbb{C}): A:=\begin{bmatrix} a & b \\ c & d\end{bmatrix} : \sum a_{ij} = 0 \wedge A \text{ is row-reduced}$. Prove that there are only three such matrices.**
    ...
    In this sense, we can give values to $a,b,c,d$ and apply the restrictions of the row-reduced form and the premise $\sum a_{ij} = 0$, giving at the end:

    $$\begin{bmatrix} 1 & -1 \\ 0 & 0  \end{bmatrix},  \begin{bmatrix} 0 & 0 \\ 1 & -1 \end{bmatrix}, \begin{bmatrix} 0 & 0 \\ 0 & 0 \end{bmatrix}$$

    The rest of the forms do not verify the condition.
...
which are two row-reduced forms of row-equivalent matrices, thus both are in the same equivalence class and the linear system they represent is the same.
...
3. If $1,...,r$ are the non-zero rows of $R$ and the first non-zero entry is in the column $k_i  : i=1,...,r$, then $k_1 < \cdots < k_r$.
...
3. **Describe explicitly all $2 \times 2$ row-reduced echelon matrices.**
    ...
    Now, we apply the second condition, and we only get the following matrices, eliminating those that have any 0-row above:
...
8. **Let be**
    ...
    **For which triples $(y_1,y_2,y_3)$ does the system $AX = Y$ have a solution**
    ...
    Thus, the system has a solution and no constraint at all for the 3-tuple.
...
9. **Let be**
    ...
    **For which $(y_1, y_2, y_3, y_4)$ does the system of equations $AX = Y$ have a solution?**
    ...
    Thus, the system has a solution when $(y_1,y_2,y_3,y_4) \in \mathbb{Q}^4$ satisfies:
...
This gives us a first approach to what the product is and what its elements are; $AX$ denotes a column of linear combinations of the elements of $X$ by the items of $A$. 
...
As we did to see the behaviour of the identity matrix, let's take a closer look at how $E\_\theta$ behaves. For simplicity, let's take $A \in M\_{2 \times 2}(F)$

$$A := \begin{pmatrix}a_{11} & a_{12} \\ a_{21} & a_{22} \end{pmatrix}, \quad E^1_{21} := \mathcal{E}^1_{21}(I_2) = \begin{pmatrix}2 & 0 \\ 0 & 1 \end{pmatrix}$$

This way, when we perform the product we have:
...
1. **Find two different $2 \times 2$ matrices A such that $A^2 = 0$ but $A\neq 0$.**
    ...
    Thus, assuming the decision that no point can be $0$, we get:
...
- A $B \in M_n(F) : AB = I_n$ is called a *right inverse* of $A$. 
...
So when the left and the right inverse exist, both matrices are the same and in general the matrix is two-sided invertible. We then call it $A^{-1}$:
...
By the last result of the section above we know that a product of invertible matrices is an invertible matrix; since $A$ is invertible $R$ must be too, but observe that this is only possible if $R$ has no zero-rows, otherwise no product could produce $I$, thus $R = I$ and $A \in [I\_n]$
...
With this characterization, we can extend our characterization of row-equivalent matrices. If any product of elementary matrices can be abstracted in an invertible matrix $P$, then for $A,B \in M\_{m \times n} (F)$:
...
Observe that by definition, the product of elementary matrices that, multiplied by $A$, gives $I_n$ is in fact $A^{-1}$, so to speak:
...
- **Corollary: square matrix with one-sided inverse is invertible (§6.3.3):** If $A \in M\_n(F)$ has a left or right inverse, then $A$ is invertible and that one-sided inverse is the two-sided inverse $A^{-1}$.
