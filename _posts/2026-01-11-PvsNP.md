...
$$ g = gcd(a,b) \iff g \in \mathbb{N^*} \ : \begin{cases}
   ...
   \ \nexists \ d \in \mathbb{N^*} : d  \mid   a , d  \mid   b  \land   d > g
\end{cases}  $$
...
$$ \exists (q_2,r_2) \in \mathbb{N}^2 : b = r_1q_2 + r_2 \ \land \ b > r_1 > r_2 \geq 0 $$
...
Also, consider from above that every divisor of $a$ and $b$ is also divisor of the remainder: $$\forall d \in \mathbb{Z}:d \mid  a \land d \mid  b \implies d \mid  (a \ \% \ b)$$

So, as also happens that if we call $r_1 = a \ \% \ b $:

$$ d  \mid   (a \ \% \ b)  \implies d  \mid   (b \ \% \ r_1) $$
...
- We also consider the set $\Sigma^*$ of all posible strings of symbols of $\Sigma$ (including the empty string $\epsilon$). This way, the set of all possible "binary numbers" would be: $$ \Sigma^* = \{0,1\}^* = \{\epsilon,0,1,10,11,110,111,1110,1111,...\}$$
    ...
    Considering $x \in \Sigma^\*$ we define as the *length* of $x$, $ \mid x\mid \ \in \mathbb{N}$; as the number of symbols $\sigma \in \Sigma$ that constitutes that string.
...
$$ L_{\Pi} = \{x \in \Sigma^*  \mid \ \Pi(x)= 1\} \subseteq \Sigma^*$$
...
```
                          ...
                          └─────────────┬────────────────────┘
                                        ...
                                        │ 
                                        ▼
    (Infinite Tape)     …  \mid   σ  \mid   a  \mid   σ  \mid   σ  \mid   b  \mid   σ  \mid   σ  \mid   a  \mid   σ  \mid   σ  \mid   b  \mid   c  \mid   c  \mid   a  \mid   …
                                        ▲
                                        │
                                  ┌─────┴───────────┐
                                  │  Tape Head      │
        ...
        DIAGRAM A — “Read” (the configuration that determines δ’s input)
                         ...
                         Finite Control Unit (states Q)
                          ...
                          └─────────────┬────────────────────┘
                                        ...
                                        ▼
        (Infinite Tape)     …  \mid   σ  \mid   σ  \mid   a  \mid   σ  \mid   σ  \mid   …
                                        ▲
                                        │
                                   ┌────┴──────┐
                                   │ Tape Head │
        ...
        DIAGRAM C — “Write” (overwrite the scanned cell with b)
                            ...
                            Finite Control Unit (states Q)
                             ...
                             └─────────────┬──────────────────┘
                                           │
                                           ▼
          (Infinite Tape)      …  \mid   σ  \mid   σ  \mid   b  \mid   σ  \mid   σ  \mid   …
                                           ▲
                                           │
                                      ┌────┴──────┐
                                      │ Tape Head │
...
The difference between those two is that in NTMs, $\delta$ is a *relation*, which means that for each input, the output is a subset of the codomain with the posibility that is greater than one: $$x \in X, \ y = \delta(x) \subset Y : \ \mid \delta(x)\mid  \ \leq  \ \mid Y\mid$$ where $ \mid \delta(x)\mid $ is $\delta(x)$'s cardinality (the number of elements of the set). 

While in DTM, $\delta$ is a *function* (a partial function to be accurate), which is a restricted subset of the relations. It means that for each input of the domain $x \in X$, $\delta$ maps at most (because it could be zero elements to map; partial) with one output $$y = \delta(x) \subset Y : \ \mid\delta(x) \mid  \ \leq \ 1$$ 
    ...
    ```
                         ...
                         Finite Control Unit (states Q)
                          ...
                          └────────────┬─────────────────────┘
                                       │
                                       ▼
         (Infinite Tape)     …  \mid     \mid     \mid   h  \mid   o  \mid   l  \mid   a  \mid     \mid     \mid   …
    ```
...
- When the *tape head* finds an empty cell, it writes an $a$, changes the state to $q_{acc}$ and moves to the right.
    ...
    ```
                          ...
                          └────────────────┬───────────────────┘
                                           │
                                           ▼ > ▼ (next cell to read)
         (Infinite Tape)    …  \mid   o  \mid   l  \mid   a  \mid     \mid     \mid     \mid     \mid     \mid    …
                          ...
                          └────────────────────┬─────────────────┘
                                               │
                                               ▼
         (Infinite Tape)    …  \mid   o  \mid   l  \mid   a  \mid   a  \mid     \mid     \mid     \mid     \mid   …
    ```
...
- $\Sigma = \Set{h,o,l,a}$, $\Sigma^*=\Set{\epsilon,h,hh,ho,hola,la,a,...}$
- $\Gamma = \Set{\square} \bigcup \ \Sigma$
- $\delta : \bigl(Q \setminus \Set{q_{\mathrm{acc}}, q_{\mathrm{rej}}}\bigr)\times \Gamma \to Q \times \Gamma \times \Set{L, R}$ satisfying:
...
$$C_i(x) \vdash_{M} C_j(x) \iff \delta(q,t(n)) = (q',t'(m-1), R) \ \vee \ \delta(q,t(n)) = (q',t'(m+1), L)\ : \ D \in \Set{L,R} $$
...
C_j = (q',t', m)
                          ...
                          └────────────────┬──────────────────┘
                                           │ 
                                           ▼ 
         (Infinite Tape)     …  \mid     \mid     \mid   j  \mid   o  \mid   l  \mid   a  \mid     \mid     \mid   …
...
where $C_0(x)$ is the initial configuration, $\vdash_{M}$ is the “one-step yields” relation, and $C_t(x)$ is halting, this is usually expresed as $C_t(x) = (q_{acc}/q_{rej},t,n): t \in T_M, n \in \mathbb{Z} $. 

A little parenthesis, we usually refers that, for a given input $x \in \Sigma^*$, we say that: 
...
$$t_{M}(n)=\max\{\,\mathrm{Time}_{M}(x) :  \mid  x \mid  =n\}$$
...
- But there is a more general and realistic concept of time for NTMs, universal over branches, which is considerate the number of steps of the largest branch (without consider if it halts or not):$$\mathrm{Time}_{M}(x)=\max\{\,p\in\mathbb{N} : C_{0}(x)\ \vdash_{M}^{\,p}\ C_{p}(x)\}$$
    ...
    $$t_{M}(n)=\max\{\,\mathrm{Time}_{M}(x) : x\in\Sigma^{*},\  \mid  x \mid  =n\,\}$$
...
As a reminder, we was saying that a decision problem over an alphabet $\Sigma$ can be understanded as Boolean function $\Pi: \Sigma^* \to \{0,1\}$ whose solution eventually lands over a language $L_{\Pi} = \{x \    \mid   \ \Pi(x)=1\} \subseteq \Sigma^*$. This way, solving $\Pi$ becomes deciding the membership of $L_\Pi$, which could be decided by a Turing Machine. That is the formal connection: a decision problem is (identified with) a language and a TM solves it by being a decider for that very language.
...
We say that $\Pi$ is solvable or has a solution if $\exists M \in DTM: M \text{ decides } L_\Pi$ (if exists a Turing Machine that decides his solution), we consider the set $M_\Pi = \{ M \  \mid   \ M \text{ decides } L_\Pi \} \subseteq DTM$. 
...
Is worth to note that all the times definition we provided is not are not fixed values, they depends on the input size; $ \mid  x \mid  $. How the value of this time grows as the length of the input grows is a subject called *Time Complexity*, and it will be discussed in the next section.
...
| Linearithmic | $n · log \ n$ | “Divide-and-conquer + linear combine” pattern | Merge sort, heap sort; many FFT-like routines                             \mid  
...
measured as a function of the input length $ n =  \mid   x  \mid   $.
...
$$DTIME(T(n)) = \{L \subseteq \Sigma^* \  \mid   \ \exists M \in DTM: \big( M \ decides \ L \land \ t_M(n) \in \mathcal{O}(T(n))\big) \}$$

$$NTIME(T(n)) = \{L \subseteq \Sigma^* \  \mid   \ \exists M \in NTM: \big( M \ decides \ L \land \ t_M(n) \in \mathcal{O}(T(n))\big) \}$$
...
- Continue up to most $T( \mid  x \mid  )$ steps and accept iff an accepting configuration is reached.

So we have demonstrated that $w$ is a piece of encoding information polynomially bounded in $ \mid  x \mid  $.
...
$$ \forall x \in L \  \exists w :  \mid  w \mid   \in \mathcal{O}( \mid  x \mid  ^k) \ \land \ V(x,w) = 1$$

The $ \mid  w \mid   \in \mathcal{O}( \mid  x \mid  ^k)$ condition refers that the encoding uses an reasonable amount of space resources.

And ultimately we can say that $V$ is a verifier of $L$: 

$$ \forall x\in \Sigma^* \ \big( x\in L \iff \exists w :  \mid  w \mid   \in \mathcal{O}( \mid  x \mid  ^k) \ \land \ V(x,w) = 1 \big)$$

Observe that also is: $$x \notin L \ \iff \ \forall w\ \bigl( \mid  w \mid  \le p( \mid  x \mid  )\ \Rightarrow\ V(x,w)=0\bigr)$$
...
- $\mathbf{NP}= \{ \ L \  \mid   \  \forall x \ \big( x\in L \iff \exists w :  \mid  w \mid   \in \mathcal{O}( \mid  x \mid  ^k) \ \land \ V(x,w) = 1 \big)\}$; Which are the class of the languages that can be verified in polynomial time.
...
Then, consider the halting computation $C_0(x) \vdash_M C_1(x) \vdash_M ... \vdash_M C_p(x)$, then we would write $C_i(x) = (q_i,t_i,n_i)$. We define $$Space_M(x):=  \mid  \{\ n_i \  \mid   \ 0 \leq i \leq p\ \} \mid  $$
...
Then, for a function $s:\mathbb{N} \to \mathbb{N}$: $$DSPACE(s(n)) = \{ L \subseteq \Sigma^* \  \mid   \ \exists \ M \in DTM \ deciding \ L: space_M(x) \in s( \mid  x \mid  )\}$$ this is, the set of those lanmguages for which exists a DTM that have a usespace function upper bounded by $s( \mid  x \mid  )$.
