...
layout: post
title: "P vs NP."
subtitle: "Turing Machines and Complexity Classes. P vs NP asymmetry."
...
3. Complexity classes: P, NP and PSPACE. PvsNP problem.
    ...
    - 3.1 Certificate/verifier characterization of NP. Verification.
        ...
        - 3.1.3. Generalizing V as a verifier of L.
...
These are all of those problems that can be decided with a reasonable amount of space.

The relation with the other classes discussed before is:
...
- *Given a starting chess-state, find the optimal chess move*. If we ask a computer about the optimal next move for a given position, the computer gives a straight answer based on a calculation of all the future game-states from the given position. There is no efficient way to check the best move, and the more you optimize the calculation about the move, the more resources you need to invest.
...
Now that we have a solid understanding of what P and NP are, let's face some core questions about complexity theory.

- First, we did see that $$P$$ is the class of problems for which there exists a deterministic solution running in polynomial time (efficient time), 

- Then, we saw that $$NP$$ is the class of those languages that admit a non-deterministic but nevertheless time-efficient solution, and we also saw that a deterministic perspective of NP problems is needed. That was the characterization of NP as the class of those languages that admit a verification in polynomial time.

Now, a convenient question is put on the table. Do those quickly verifiable problems also admit an efficient solution? Or said in other words, is NP part of P? 

$$ P \supseteq NP $$

Remember that above we said that $$ P \subseteq NP $$; both statements together are equivalent to proposing the equivalence between those two sets:

$$ P \subseteq NP \land P \supseteq NP \iff P = NP $$

This is, *any problem that can be efficiently verified can also be efficiently decided*?
...
As a summary, we dedicated a part of the course to understanding the PvNP subject (along with Turing machines, efficient time-solving problems, etc.) because ZK cares about the verifying aspect of computation, this means, the subject of developing a witness that efficiently proves a YES-instance of a problem.