...
Thus, that $$a \vert r_2 $$ and also is evident that $$b | r_2 $$ so this number meets the first condition.
...
- We also consider the set $$\Sigma^*$$ of all posible strings of symbols of $$\Sigma$$ (including the empty string $$\epsilon$$). This way, the set of all possible "binary numbers" would be: 
    ...
    Considering $$x \in \Sigma^*$$ we define as the *length* of $$x \test { as } \vert x \vert \in \mathbb{N}$$; as the number of symbols $$\sigma \in \Sigma$$ that constitutes that string.
...
where $$|\delta(x)|$$ is $$\delta(x)$$'s cardinality (the number of elements of the set). 
...
this is, the set of those lanmguages for which exists a DTM that have a usespace function upper bounded by $$s(|x|)$$.
